...
def upload_files(sc):
    ...
    try:
        if(isinstance(rds_connection,rds.RDS) and isinstance(local_connection,rds.Local)):
            ...
            logging.error('Exception in controller upload_files() function. Cannot obtain local/cloud db connection '+str(e))
    except Exception as e:
        logging.error("Exception in controller upload_files() function "+str(e))
    finally:
        sc.enter(file_freq, 1, upload_files, (sc,))
    ...
    try:
        ...
        try:
            if(isinstance(rds_connection,rds.RDS) and isinstance(local_connection,rds.Local)):
                ...
                logging.error("Exception in controller upload_db() function cannot obtain local/cloud db connection  "+str(e))
        except Exception as e:
            print('Exception in controller ' + str(e))
...
def log_missing_data(sc):
    ...
    try:
        if(isinstance(rds_connection,rds.RDS) and isinstance(local_connection,rds.Local)):
            ...
            logging.error("Exception in controller log_missing_data() function cannot obtain local/cloud db connection  "+str(e))
    except Exception as e:
